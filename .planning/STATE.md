...
- [Security 2026-06-24]: GAM-002/003/004 resolvidos na migration 20260624000012 (aplicada em prod) + Edge Function redeployada (v2): GAM-002 crédito de resultado exige image_url não-vazio (anti-farm); GAM-003 cap diário + spend atômicos via nova fn spend_generation_credit (advisory lock único, sem TOCTOU) — Edge Function agora chama spend_generation_credit em vez de count+spend_credit; GAM-004 level-up credita +2 por nível cruzado. Todos os achados do review (GAM-001..005) agora fechados. Pendente só re-rodar o review automatizado (limite de sessão).
